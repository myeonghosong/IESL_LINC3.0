...
int main() {
    ...
    const char *query = "SELECT rcv.bike_id, rcv.date_time, rcv.dong, rcv.gu, rcv.city, rcv.error, user.start_time, user.start_dong, user.start_gu, user.start_city FROM rcv_data_all AS rcv INNER JOIN user_location AS user ON rcv.bike_id = user.user_id";
    ...
    // 데이터를 배열에 저장
    for (int i = 0; i < numSamples; i++) {
        usageData[i].time = atoi(strrchr(PQgetvalue(res, i, 1), ':') + 1);
        strcpy(usageData[i].dong, PQgetvalue(res, i, 2));
        strcpy(usageData[i].gu, PQgetvalue(res, i, 3));
        strcpy(usageData[i].city, PQgetvalue(res, i, 4));
        usageData[i].usage = atof(PQgetvalue(res, i, 5));
    }
